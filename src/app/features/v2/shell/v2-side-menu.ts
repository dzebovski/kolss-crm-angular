...
// Items come from `V2_NAV_ITEMS`; actions are handled by the shell.
@Component({
...
export class V2SideMenu {
  ...
  readonly action = output<V2NavAction>();
  ...
  protected readonly languageName = computed(
    () => V2_LANGUAGES.find((language) => language.code === this.session.locale())?.name ?? '',
  );
}
