...
describe('RemindersPage lead workflow', () => {
  ...
  it('opens the detail drawer without navigating away from reminders', async () => {
    const { dialogOpen, fixture } = await render();
    const router = TestBed.inject(Router);
    (fixture.nativeElement as HTMLElement).querySelector<HTMLButtonElement>('.lead-open')!.click();
    await fixture.whenStable();
  ...
  it('restores the list position and lead focus after a dirty close', async () => {
  ...
  it('has no automated accessibility violations in the reminder list', async () => {
    ...
    const reminders = fixture.nativeElement.querySelector('.reminders')!;
    expect((await axe.run(reminders)).violations).toEqual([]);
  });

  it('shows selected dates next to callback and waiting statuses', async () => {
    ...
    const meta = fixture.nativeElement.querySelector('.lead-meta');
